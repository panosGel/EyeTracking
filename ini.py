DATASET_FOLDER = 'C:\\Users\\panos\\PycharmProjects\\eyeTracking\\eyeTracking\\datasets\\'
IMAGE_FOLDER = 'C:\\Users\\panos\\PycharmProjects\\eyeTracking\\eyeTracking\\imageFolder'
ALLOWED_EXTENSIONS = set(['tsv', 'xml'])
DATABASE = 'C:\\Users\\panos\\PycharmProjects\\eyeTracking\\eyeTracking\\database\\flask.db'
VISUALIZER_FOLDER = 'C:\\Users\\panos\\PycharmProjects\\eyeTracking\\eyeTracking\\visualizerFolder\\'
VISUALIZER_FOLDER_FAMILIARITY = 'C:\\Users\\panos\\PycharmProjects\\eyeTracking\\eyeTracking\\visualizerFolder\\familiarityResults'
GAZE_DATA = "C:\\Users\\panos\\PycharmProjects\\eyeTracking\\eyeTracking\\gazeData\\"
STATS_FOLDER = "C:\\Users\\panos\\PycharmProjects\\eyeTracking\\eyeTracking\\viewerStatistics\\"

PARTICIPANTS_FOLDER = 'C:\\Users\\panos\\PycharmProjects\eyeTracking\eyeTracking\\visualizerFolder\\participantsVisualizations\\'
#separated datasets depending on gender
FEMALE_PARTICIPANTS = []
MALE_PARTICIPANTS = []

#separated datasets depending on agegroup
AGEGROUP_1_PARTICIPANTS = []
AGEGROUP_2_PARTICIPANTS = []
AGEGROUP_3_PARTICIPANTS = []
AGEGROUP_4_PARTICIPANTS = []
AGEGROUP_5_PARTICIPANTS = []

#separated datasets depending on level of interest
INTEREST_GROUP_1_VERY_INTERESTED = []
INTEREST_GROUP_2_QUITE_INTERESTED = []
INTEREST_GROUP_3_MODERATE_INTEREST = []
INTEREST_GROUP_4_NOT_VERY_INTERESTED = []

#separated datasets depending on familiarity

#"Cheviot farm"
CHEVIOT_FARM_1=[]
CHEVIOT_FARM_2=[]
CHEVIOT_FARM_3=[]

#"'Now for the Painter' (Rope) - Passengers Going on Board "

NOW_FOR_THE_PAINTER_1=[]
NOW_FOR_THE_PAINTER_2=[]
NOW_FOR_THE_PAINTER_3=[]

#"The Fortress of Konigstein: Courtyard with the Brunnenhaus"
KONIGSTEIN_1=[]
KONIGSTEIN_2=[]
KONIGSTEIN_3=[]

#"Woman and suspended man"
WOMAN_AND_SUSPENDED_MAN_1=[]
WOMAN_AND_SUSPENDED_MAN_2=[]
WOMAN_AND_SUSPENDED_MAN_3=[]

#"14-06-1964"
FOURTEEN_SIX_1964_1=[]
FOURTEEN_SIX_1964_2=[]
FOURTEEN_SIX_1964_3=[]

#"Cheetah and stags with two indians"
CHEETAH_STAGS_1=[]
CHEETAH_STAGS_2=[]
CHEETAH_STAGS_3=[]

#"Flask Walk, Hampstead"
FLASK_WALK_1=[]
FLASK_WALK_2=[]
FLASK_WALK_3=[]

#"Release"
RELEASE_1=[]
RELEASE_2=[]
RELEASE_3=[]

#"Self-portrait"
SELF_PORTRAIT_1=[]
SELF_PORTRAIT_2=[]
SELF_PORTRAIT_3=[]

#"When the West with Evening Glows"
WEST_WITH_EVENING_1=[]
WEST_WITH_EVENING_2=[]
WEST_WITH_EVENING_3=[]

#"Sir Gregory Page-Turner"
SIR_GREGORY_1=[]
SIR_GREGORY_2=[]
SIR_GREGORY_3=[]

#"rhyl sands"
RHYL_SANDS_FAMILIARITY_1 = []
RHYL_SANDS_FAMILIARITY_2 = []
RHYL_SANDS_FAMILIARITY_3 = []






#